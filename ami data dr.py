...
def older_than_one_month(date):
    get_date_obj = parse(date)
    date_obj = get_date_obj.replace(tzinfo=None)
    return date_obj < datetime.datetime(2022, 5, 24)
...
for im in ims['Images']:
    ...
    day_old = older_than_one_month(create_date)
    ami_id = im['ImageId']
    ami_name = im['Name']
    name = ""
    try:
    ...
    if day_old and 'awsbackup' not in ami_name.lower() and "golden" not in ami_name.lower() and not ('not' in ami_name.lower() and 'delete' in ami_name.lower()) \
