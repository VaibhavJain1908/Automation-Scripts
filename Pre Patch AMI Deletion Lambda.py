    ...
    try:
        ...
        for region in ["us-east-1", "us-west-1"]:
            ...
            for im in ims['Images']:
                ...
                if day_old > age and 'pre' in ami_name.lower() and 'patch' in ami_name.lower() and "golden" not in ami_name.lower() and not ('do' in ami_name.lower() and 'not' in ami_name.lower() and 'delete' in ami_name.lower()) \
        ...
        try:
            s3_client.upload_file('/tmp/' + file_name, 'lw-ami-deletion', 'Prod/PrePatch/' + str(today.year) + '/' + str(today.month).rjust(2, '0') + '/' + str(today.day).rjust(2, '0') + '/' + file_name, ExtraArgs={'ACL': 'bucket-owner-full-control'})
            print('Upload done !!') 
            ...
            response = sns_client.publish(
                TopicArn='arn:aws:sns:us-east-1:680442571075:AMI_Deletion',
                Message=message + table(data),
                Subject="Pre Patch AMI Deletion !!",
            )['MessageId']
