...
def lambda_handler(event, context):
    try:
        ...
        try:
            s3_client.upload_file('/tmp/' + file_name, 'lw-ami-deletion', 'Dev/PrePatch/' + str(today.year) + '/' + str(today.month).rjust(2, '0') + '/' + str(today.day).rjust(2, '0') + '/' + file_name, ExtraArgs={'ACL': 'bucket-owner-full-control'})
            print('Upload done !!') 
