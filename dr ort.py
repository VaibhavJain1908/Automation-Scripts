...
for instance in instances:
    ...
    for tag in instance.tags:
        ...
        elif tag["Key"] == "CoreApplicationGroup":
            service_statuses[i]["CoreApplicationGroup"] = tag["Value"]
        elif tag["Key"] == "Department":
            service_statuses[i]["Department"] = tag["Value"]
        elif tag["Key"] == "ServerType":
            service_statuses[i]["ServerType"] = tag["Value"]
        elif tag["Key"] == "Server Type":
            service_statuses[i]["Server Type"] = tag["Value"]
        elif tag["Key"] == "map-sap":
            service_statuses[i]["map-sap"] = tag["Value"]
    ...
    try:
        if flag == 0:
            ...
            response = ssm_client.send_command(
                        InstanceIds=[instance.id],
                        DocumentName="AWS-RunShellScript",
                        Parameters={'commands': ["cat /etc/sudoers.d/30-aws_basis_administrators"]}, )
            
            command_id = response['Command']['CommandId']
            time.sleep(2)
            
            print("Currently checking sudoers entries !!")
            output = ssm_client.get_command_invocation(
                  CommandId=command_id,
                  InstanceId=instance.id
                )
            service_statuses[i]['sudoers entries'] = output['StandardOutputContent']
            ...
            service_statuses[i]['last patched evidence'] = output['StandardOutputContent']
            ...
            response = ssm_client.send_command(
                        InstanceIds=[instance.id],
                        DocumentName="AWS-RunShellScript",
                        Parameters={'commands': ["zypper lu"]}, )
            
            command_id = response['Command']['CommandId']
            time.sleep(120)
            
            print("Currently checking if the server is patched", end=" - ")
            output = ssm_client.get_command_invocation(
                  CommandId=command_id,
                  InstanceId=instance.id
                )
            service_statuses[i]['patched evidence'] = output['StandardOutputContent']
            if "No updates found." in output['StandardOutputContent']:
                service_statuses[i]['patched'] = "Yes"
            else:
                service_statuses[i]['patched'] = "No"
            print(service_statuses[i]['patched'])
            ...
            for line in output['StandardOutputContent'].splitlines():
                ...
                elif "nofail" not in words[3] and "/" != words[1] and "/boot" != words[1] and "swap" != words[1] and "efs" not in words[0]:
                    print("Wrong Fstab entry - " + line)
                    service_statuses[i]["Wrong entry"] = line
                    service_statuses[i]["Valid Fstab"] = "No"
                    break
...
fields = ['Instance Name', 'Instance Id', 'IP Address', 'Region', 'Instance Type', 'OS', 'Application', 'CoreApplicationGroup', 'Department', 'map-sap', 'ServerType', 'Server Type', 'Environment', 'environment', 'supervisory-organization', 'company', 'cost-center', 'Backup', 'splunk', 'splunk evidence', 'ntpd', 'ntpd evidence', 'chronyd', 'chronyd evidence', 'auditd', 'auditd evidence', 'amazon-ssm-agent', 'amazon-ssm-agent evidence', 'ds_agent', 'ds_agent evidence', 'sudoers entries', 'patched', 'patched evidence', 'last patched', 'last patched evidence', 'Valid Fstab', 'Wrong entry']
